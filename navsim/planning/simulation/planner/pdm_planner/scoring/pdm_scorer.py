...
class PDMScorerConfig:
    ...
    lane_keeping_deviation_limit: float = 0.5  # [m] (lane keeping)
    lane_keeping_horizon_window: float = 2.0  # [s] (lane keeping)
...
class PDMScorer:
    ...
    def score_proposals(
        ...
        multiplicative_metrics_prods = self._multi_metrics.prod(axis=0)
    ...
    def _aggregate_pdm_scores(self) -> npt.NDArray[np.float64]:
        ...
        # exclude TWO_FRAME_EXTENDED_COMFORT from weighted sum
    ...
    def _reset(
        ...
        self._ego_areas = np.zeros(
            (self._num_proposals, self.proposal_sampling.num_poses + 1, len(EgoAreaIndex)),
            dtype=np.bool_,
        )
