...
from navsim.common.dataloader_navhard import MetricCacheLoader, SceneFilter, SceneLoader
...
def run_pdm_score(args: List[Dict[str, Union[List[str], DictConfig]]]) -> List[pd.DataFrame]:
    ...
    img_perturb = instantiate(cfg.img_perturb) if cfg.get("img_perturb") is not None else None
    scene_loader = SceneLoader(
        synthetic_sensor_path=Path(cfg.synthetic_sensor_path) if cfg.synthetic_sensor_path is not None else None,
        original_sensor_path=Path(cfg.original_sensor_path) if cfg.original_sensor_path is not None else None,
        data_path=Path(cfg.navsim_log_path),
        synthetic_scenes_path=Path(cfg.synthetic_scenes_path) if cfg.synthetic_scenes_path is not None else None,
        scene_filter=scene_filter,
        sensor_config=agent.get_sensor_config(),
        img_perturb=img_perturb,
    )

    pdm_results: List[pd.DataFrame] = []

    # first stage — use log-replay (non-reactive) when not evaluating synthetic scenes
    use_reactive = cfg.train_test_split.scene_filter.get("include_synthetic_scenes", False)
    stage_one_policy_cfg = cfg.traffic_agents_policy.reactive if use_reactive else cfg.traffic_agents_policy.non_reactive
    traffic_agents_policy_stage_one: AbstractTrafficAgentsPolicy = instantiate(
        stage_one_policy_cfg, simulator.proposal_sampling
    )
    ...
    for idx, (token) in enumerate(tokens_to_evaluate_stage_one):
        ...
        try:
            ...
            # file_path structure: cache_root/{log_name}/{scene_type}/{token}/metric_cache.pkl
            _parts = metric_cache.file_path.parts
            score_row_stage_one["log_name"] = getattr(metric_cache, "log_name", _parts[-4])
            score_row_stage_one["frame_type"] = getattr(metric_cache, "scene_type", _parts[-3])
            _timepoint = getattr(metric_cache, "timepoint", metric_cache.ego_state.time_point)
            score_row_stage_one["start_time"] = _timepoint.time_s
            end_pose = StateSE2(
...
def main(cfg: DictConfig) -> None:
    ...
    # Override output_dir if run_name is provided
    if cfg.get("run_name") is not None:
        exp_root = os.environ.get("NAVSIM_EXP_ROOT", "")
        cfg.output_dir = f"{exp_root}/{cfg.experiment_name}/{cfg.run_name}"
    ...
    # For two-stage evaluation, we need to ensure complete groups are included
    ...
    if max_scenarios is not None and max_scenarios > 0:
        ...
        # Check if this is a two-stage evaluation (has reactive_all_mapping)
        raw_mapping = cfg.train_test_split.get("reactive_all_mapping", None)

        if raw_mapping is not None:
            # Smart token selection: include complete two-stage groups
            logger.info("Two-stage evaluation detected. Selecting complete groups for max_scenarios limit.")
            ...
            for orig_token, prev_token, two_stage_pairs in raw_mapping:
                ...
                group_tokens = {orig_token, prev_token}
                for t1, t2 in two_stage_pairs:
                    group_tokens.add(t1)
                    group_tokens.add(t2)

                # Only include tokens that are in tokens_to_evaluate
                group_tokens = group_tokens & set(tokens_to_evaluate)

                if len(selected_tokens) + len(group_tokens) <= max_scenarios:
                    selected_tokens.update(group_tokens)
                    group_count += 1
                else:
                    break

            tokens_to_evaluate = list(selected_tokens)
            logger.info(
                f"Selected {len(tokens_to_evaluate)} tokens forming {group_count} complete two-stage groups "
                f"(out of {original_count} available tokens)"
            )
        else:
            # Simple truncation for non-two-stage evaluation
    ...
    if raw_mapping is None:
        logger.info("No reactive_all_mapping in train_test_split config — using single-stage scoring.")
        pdm_score_df["weight"] = 1.0
    else:
        try:
            # Get tokens that successfully evaluated (valid=True and have required fields)
            evaluated_tokens = set(pdm_score_df[pdm_score_df["valid"] == True]["token"].tolist())

            for orig_token, prev_token, two_stage_pairs in raw_mapping:
                # BOTH first-stage tokens must have succeeded
                if orig_token not in evaluated_tokens or prev_token not in evaluated_tokens:
                    continue

                # Filter second-stage pairs to only include successful tokens
                filtered_pairs = [
                    (t1, t2) for t1, t2 in two_stage_pairs
                    if t1 in evaluated_tokens and t2 in evaluated_tokens
                ]
                # Only add if there are valid second-stage pairs
                if filtered_pairs:
                    all_mappings[(orig_token, prev_token)] = filtered_pairs

            logger.info(
                f"Two-stage filtering: {len(evaluated_tokens)} valid tokens, "
                f"{len(all_mappings)}/{len(raw_mapping)} complete groups"
            )

            if not all_mappings:
                raise ValueError(
                    f"No complete two-stage groups found. {len(evaluated_tokens)} valid tokens "
                    f"but none form complete pairs. Data may be corrupted or incomplete."
                )
    ...
    # Only split into stage_one/stage_two for successful two-stage (navhard) evaluation
    if pseudo_closed_loop_valid and "frame_type" in pdm_score_df.columns:
    ...
    if pseudo_closed_loop_valid:
        ...
        # No two-stage mapping available; fall back to simple mean of pdm_score
        valid_mask = pdm_score_df["valid"] == True
        score_col = "score" if "score" in pdm_score_df.columns else "pdm_score"
        combined_row["score"] = pdm_score_df.loc[valid_mask, score_col].mean() if valid_mask.any() else np.nan
