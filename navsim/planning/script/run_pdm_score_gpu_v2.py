...
# ADDED FOR DRIVOR TO RUN NAVHARD_TWO_STAGE!

import sys
# Put DrivoR's local navsim first so all navsim.* imports resolve to it,
# not the conda-env's editable Navsim install in site-packages.
_DRIVOR_ROOT = "/fs/nexus-projects/sim2real/aliu/DrivoR"
if _DRIVOR_ROOT not in sys.path:
    sys.path.insert(0, _DRIVOR_ROOT)
...
from navsim.planning.training.agent_lightning_module_drivor import AgentLightningModule
...
def main(cfg: DictConfig) -> None:
    ...
    dataset = Dataset(
        ...
        target_builders=[],  # targets unused in predict_step; avoids crash on synthetic scenes with num_future_frames=0
        cache_path=None,
